...
def talk(what, how):
   ...
   n = 0
   a = what.split();
   while ((len(a) - n) >= MAX_WORDS):
      string = '+'.join(a[n:n+MAX_WORDS])  
      final_string = "http://translate.google.com/translate_tts?tl=" + how + "&q=" + string
      try: 
         subprocess.call(["mpg123", final_string])
      except OSError:
         print "Please make sure 'mpg123' is installed. ",
         print "[(Debian cmd): sudo apt-get install mpg123]"
         exit(1)
...
if __name__ == "__main__":
   try:
      subprocess.check_call(['which', 'mpg123'])
